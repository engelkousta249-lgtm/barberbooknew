...
const PLAN = { key:"freemium", name:"Freemium", price:"€0", maxBarbers:1 }
  ...
  async function handleFinish() {
    ...
    try {
      ...
      const { data: shop, error: shopErr } = await supabase.from("barbershops").insert({
        ...
        logo_url: logoUrl, num_barbers: 1, plan: "freemium",
      ...
      await fetch("/api/send-email", {
        ...
        body: JSON.stringify({type:"welcome_owner", to:existingUser?.email||email, data:{ownerName:shopName, shopName, city}})
      })
    ...
    <>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        .logo-upload{border:2px dashed rgba(59,130,246,.25);border-radius:14px;padding:24px;text-align:center;cursor:pointer;background:rgba(59,130,246,.04);display:block;transition:all .2s;}
        .logo-upload:hover{border-color:var(--blue);}
        ...
        .copy-btn{padding:8px 14px;background:var(--blue);border:none;color:#fff;border-radius:8px;font-size:12px;font-weight:700;cursor:pointer;white-space:nowrap;}
      ...
      <nav className="nav">
        ...
        <div className="plan-chip">🆓 Freemium · €0/μήνα</div>
      </nav>
          ...
          <div className="card">
            ...
            {!done ? (
              <>
                {step===1 && (
                  <>
                    ...
                    <div className="card-sub">Ξεκίνα δωρεάν με το Freemium πλάνο</div>
                    <div className="field">
                      <label>Email</label>
                    ...
                    <div className="field">
                      ...
                      <div className="pass-wrap">
                        ...
                        <button className="pass-eye" type="button" onClick={()=>setShowPass(!showPass)}>{showPass?"🙈":"👁️"}</button>
                      </div>
                    </div>
                  </>
                )}
                    ...
                    <div className="field">
                      ...
                      {logoPreview ? (
                        <div style={{textAlign:"center"}}>
                          ...
                          <button className="btn" style={{fontSize:12,padding:"6px 14px",marginTop:8}} onClick={()=>{setLogoFile(null);setLogoPreview("")}}>Αλλαγή</button>
                        </div>
                      ) : (
                  ...
                  <>
                    ...
                    {services.map((s,i) => (
                      <div key={i} className="svc-row">
                        <input className="svc-inp" value={s.name} placeholder="π.χ. Κούρεμα" onChange={e=>{const n=[...services];n[i].name=e.target.value;setServices(n)}}/>
                        <input className="svc-inp gold" type="number" value={s.price} onChange={e=>{const n=[...services];n[i].price=+e.target.value;setServices(n)}}/>
                  ...
                  <>
                    ...
                    {hours.map((h,i) => (
                      <div key={i} className={`hour-row ${h.active?"":"off"}`}>
                        ...
                        <div className={`toggle ${h.active?"on":""}`} onClick={()=>{const n=[...hours];n[i].active=!n[i].active;if(n[i].active&&!n[i].open){n[i].open="09:00";n[i].close="19:00"}setHours(n)}}/>
                      </div>
                    ))}
                  </>
                )}
                ...
                {step===5 && (
                  <>
                    ...
                    <div className="gallery-grid">
                      {photoPreviews.map((url,i) => (
                        <div key={i} className="gallery-item">
                          <img src={url} alt=""/>
                          <button className="gallery-del" onClick={()=>{setPhotoPreviews(p=>p.filter((_,j)=>j!==i));setPhotoFiles(p=>p.filter((_,j)=>j!==i))}}>✕</button>
                        </div>
                      ))}
                      {photoPreviews.length<6 && (
                        <label className="gallery-add">
                          <span className="plus">+</span><span>Φωτογραφία</span>
                          <input type="file" accept="image/*" multiple style={{display:"none"}} onChange={e=>{
                            const files=Array.from(e.target.files||[])
                            const toAdd=files.slice(0,6-photoFiles.length)
                    ...
                    <div className="barber-box">
                      ...
                      <div className="barber-box-sub">Freemium: 1 barber μόνο</div>
                ...
                <div className="actions">
                  ...
                  <button className="btn primary" disabled={!canNext()||loading}
                    onClick={()=>{if(!canNext())return;if(step===TOTAL_STEPS)handleFinish();else setStep(s=>s+1)}}>
              ...
              <div className="success">
                ...
                <p>Το κουρείο σου είναι ζωντανό! Μοιράσου το link στο Instagram bio σου.</p>
                ...
                <div className="success-card">
                  ...
                  <div className="sc-row"><span className="sc-label">Πλάνο</span><span className="sc-val">🆓 Freemium</span></div>
