...
export default function TeamOnboarding() {
 ...
 useEffect(() => {
    ...
    if (sid) {
      supabase.from("barbershops")
        ...
        .then(() => {
          ...
          setDone(true)
          window.history.replaceState({}, "", "/onboarding/team")
        })
    }
  }
  ...
  function canNext() {
    ...
    if (step===5) return barbers.filter(b=>b.name.trim()).length>=1
    return true
  }
...
if (!authData.user?.id) { setError("Σφάλμα εγγραφής!"); setLoading(false); return }
      ...
      const { data: shop, error: shopErr } = await supabase.from("barbershops").insert({
        ...
        logo_url:logoUrl, num_barbers:validBarbers.length, plan:"team",
      ...
      const res = await fetch("/api/create-checkout", {
        ...
        body: JSON.stringify({ plan:"team", barbershopId:shop.id })
      })
  ...
  return (
    <>
      <style>{`
        ...
        :root{--bg:#0a0f1e;--card:#111827;--card2:#1a2235;--border:rgba(255,255,255,.07);--blue:#3b82f6;--green-main:#10b981;--green-soft:rgba(16,185,129,.12);--gold:#f59e0b;--text:#f1f5f9;--muted:#64748b;--muted2:#94a3b8;--green:#10b981;--red:#ef4444;}
        *{box-sizing:border-box;margin:0;padding:0;}
        body{font-family:'Inter',sans-serif;background:var(--bg);color:var(--text);min-height:100vh;background-image:radial-gradient(ellipse 800px 600px at 20% 0%,rgba(16,185,129,.1),transparent),radial-gradient(ellipse 600px 400px at 80% 100%,rgba(16,185,129,.06),transparent);}
        ...
        .brand{font-size:20px;font-weight:800;background:linear-gradient(135deg,var(--green),#059669);-webkit-background-clip:text;-webkit-text-fill-color:transparent;cursor:pointer;}
        .plan-chip{background:rgba(16,185,129,.12);border:1px solid rgba(16,185,129,.3);border-radius:999px;padding:5px 14px;font-size:12px;font-weight:700;color:var(--green);}
        ...
        .step-item.active{background:var(--green-soft);border-color:rgba(16,185,129,.25);}
        ...
        .step-item.active .step-icon{background:var(--green-soft);border-color:rgba(16,185,129,.3);}
        ...
        .step-item.active .step-name{color:var(--green);}
        ...
        .progress-fill{height:100%;background:linear-gradient(90deg,var(--green),#059669);border-radius:4px;transition:width .4s;}
        ...
        .card-icon{width:48px;height:48px;border-radius:14px;margin-bottom:16px;display:flex;align-items:center;justify-content:center;font-size:24px;background:var(--green-soft);border:1px solid rgba(16,185,129,.2);}
        ...
        .add-btn{width:100%;padding:10px;border-radius:10px;border:1.5px dashed rgba(16,185,129,.25);background:rgba(16,185,129,.04);color:var(--green);font-size:13px;font-weight:600;cursor:pointer;margin-top:8px;}
        ...
        .toggle.on{background:rgba(16,185,129,.25);border-color:rgba(16,185,129,.4);}
        ...
        .gallery-add{aspect-ratio:1;border-radius:12px;border:2px dashed rgba(16,185,129,.25);background:rgba(16,185,129,.04);display:flex;flex-direction:column;align-items:center;justify-content:center;cursor:pointer;color:var(--green);font-size:12px;font-weight:600;gap:6px;}
        .gallery-add .plus{font-size:24px;}
        .logo-upload{border:2px dashed rgba(16,185,129,.25);border-radius:14px;padding:24px;text-align:center;cursor:pointer;background:rgba(16,185,129,.04);display:block;}
        .logo-preview{width:80px;height:80px;border-radius:50%;object-fit:cover;border:3px solid var(--green);margin:0 auto 12px;display:block;}
        ...
        .barber-row{display:grid;grid-template-columns:1fr 1fr auto;gap:8px;align-items:center;margin-bottom:8px;padding:12px 14px;background:var(--card2);border:1px solid var(--border);border-radius:12px;}
        .barber-num{font-size:11px;color:var(--green);font-weight:700;text-transform:uppercase;letter-spacing:.5px;margin-bottom:6px;grid-column:1/-1;}
        .limit-badge{display:inline-flex;align-items:center;gap:6px;padding:6px 14px;background:rgba(16,185,129,.1);border:1px solid rgba(16,185,129,.25);border-radius:999px;font-size:12px;font-weight:600;color:var(--green);margin-bottom:16px;}
        .payment-box{background:var(--card2);border:1px solid rgba(16,185,129,.3);border-radius:16px;padding:24px;text-align:center;}
        .payment-price{font-family:'Outfit',sans-serif;font-size:48px;font-weight:900;color:var(--green);margin:12px 0 4px;}
        ...
        .payment-features li::before{content:"✓";color:var(--green);font-weight:800;font-size:16px;}
        ...
        .btn.primary{background:linear-gradient(135deg,var(--green),#059669);border:none;color:#fff;flex:1;}
        ...
        .link-box{background:rgba(16,185,129,.08);border:1px solid rgba(16,185,129,.25);border-radius:12px;padding:16px;margin-bottom:20px;display:flex;align-items:center;gap:12px;}
        .link-box input{flex:1;background:none;border:none;color:var(--green);font-size:13px;font-weight:600;outline:none;}
        .copy-btn{padding:8px 14px;background:var(--green);border:none;color:#fff;border-radius:8px;font-size:12px;font-weight:700;cursor:pointer;}
      ...
      <nav className="nav">
        ...
        <div className="plan-chip">🏆 Team · €28/μήνα</div>
      </nav>
            ...
            {!done ? (
              <>
                {step===1 && (<>
                  ...
                  <div className="card-sub">Team πλάνο · 3-10 barbers · €28/μήνα</div>
                ...
                {step===5 && (<>
                  ...
                  <div className="card-sub">Φωτογραφίες και ονόματα barbers (μέχρι 10)</div>
                  ...
                  <div className="barber-section">
                    <div className="barber-section-title">🏆 Ομάδα Team</div>
                    <div className="barber-section-sub">Πρόσθεσε από 3 έως 10 barbers</div>
                    <div className="limit-badge">👥 {barbers.filter(b=>b.name).length}/{MAX_BARBERS} barbers</div>
                    {barbers.map((b,i)=>(
                      <div key={i} className="barber-row">
                        <div className="barber-num">Barber {i+1}</div>
                        <input className="svc-inp" value={b.name} placeholder={`π.χ. Barber ${i+1}`} onChange={e=>{const n=[...barbers];n[i].name=e.target.value;setBarbers(n)}}/>
                        <input className="svc-inp" value={b.role} placeholder="Barber" onChange={e=>{const n=[...barbers];n[i].role=e.target.value;setBarbers(n)}}/>
                        {barbers.length>3&&<button className="del-btn" onClick={()=>setBarbers(p=>p.filter((_,j)=>j!==i))}>✕</button>}
                      </div>
                    ))}
                    <button className="add-btn"
                      disabled={barbers.length>=MAX_BARBERS}
                      onClick={()=>{if(barbers.length<MAX_BARBERS)setBarbers(p=>[...p,{name:"",role:"Barber"}])}}>
                      {barbers.length>=MAX_BARBERS?`✕ Μέγιστο ${MAX_BARBERS}`:`+ Barber (${barbers.length}/${MAX_BARBERS})`}
                    </button>
                  </div>
                </>)}
                ...
                {step===6 && (<>
                  ...
                  <div className="payment-box">
                    <div style={{fontSize:13,color:"var(--muted)",textTransform:"uppercase",letterSpacing:".8px",fontWeight:700}}>Team Πλάνο</div>
                    <div className="payment-price">€28</div>
                    ...
                    <ul className="payment-features">
                      ...
                      <li>Πλήρης διαχείριση ομάδας</li>
                    </ul>
                  ...
                  <div style={{marginTop:16,background:"var(--card2)",border:"1px solid var(--border)",borderRadius:12,padding:16}}>
                    ...
                    <div style={{fontSize:12,color:"var(--muted2)",display:"flex",flexDirection:"column",gap:4}}>
                      ...
                      <span>👥 {barbers.filter(b=>b.name).length} barbers</span>
                    </div>
                  </div>
                </>)}
