...
export default function TeamOnboarding() {
  ...
  async function handleFinish() {
    ...
    try {
      ...
      const { data: shop, error: shopErr } = await supabase.from("barbershops").insert({
        ...
        logo_url:logoUrl,num_barbers:validBarbers.length,plan:"team",description:"",rating:5.0,
      }).select().single()
      ...
      if (validBarbers.length>0) await supabase.from("barbers").insert(validBarbers.map(b=>({shop_id:shop.id,name:b.name,role:b.role||"Barber"})))
  ...
  const stepLabels=["Λογαριασμός","Κατάστημα","Υπηρεσίες","Ωράριο","Gallery & Ομάδα"]
  const stepIcons=["👤","💈","✂️","🕒","🖼️"]
  ...
  return (
    <>
      <style>{`
        ...
        :root{--bg:#0a0f1e;--card:#111827;--card2:#1a2235;--border:rgba(255,255,255,.07);--blue:#3b82f6;--gold:#f59e0b;--green-main:#10b981;--text:#f1f5f9;--muted:#64748b;--muted2:#94a3b8;--green:#10b981;--red:#ef4444;}
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        .step-item.active{background:rgba(16,185,129,.1);border-color:rgba(16,185,129,.25);}
        ...
        .step-item.active .step-icon{background:rgba(16,185,129,.12);border-color:rgba(16,185,129,.3);}
        ...
        .card-icon{width:48px;height:48px;border-radius:14px;margin-bottom:16px;display:flex;align-items:center;justify-content:center;font-size:24px;background:rgba(16,185,129,.12);border:1px solid rgba(16,185,129,.2);}
        ...
        .field input:focus{border-color:var(--green);box-shadow:0 0 0 3px rgba(16,185,129,.1);}
        ...
        .barber-num{font-size:11px;color:var(--green);font-weight:700;text-transform:uppercase;letter-spacing:.5px;margin-bottom:8px;grid-column:1/-1;}
        ...
        .svc-inp:focus{border-color:var(--green);}
        ...
        .add-btn{width:100%;padding:10px;border-radius:10px;border:1.5px dashed rgba(16,185,129,.25);background:rgba(16,185,129,.04);color:var(--green);font-size:13px;font-weight:600;cursor:pointer;margin-top:8px;transition:all .2s;}
        .add-btn:hover{border-color:var(--green);background:rgba(16,185,129,.08);}
          ...
          <div className="card">
            ...
            {!done ? (
              <>
                ...
                {step===5 && (<>
                  ...
                  <div className="barber-section">
                    <div className="barber-section-title">🏆 Ομάδα Barbers (Team)</div>
                    ...
                    <button className="add-btn"
                      ...
                      {barbers.length>=MAX_BARBERS?`✕ Μέγιστο ${MAX_BARBERS} barbers`:`+ Προσθήκη Barber (${barbers.length}/${MAX_BARBERS})`}
                    </button>
                  </div>
                </>)}
              ...
              <div className="success">
                ...
                <p>Το κουρείο σου είναι ζωντανό με {barbers.filter(b=>b.name).length} barbers!</p>
